...
pub async fn add_user_folder(db_pool: &Pool, user_id: i32, name: String, parent_folder_id: Option<i32>) -> Result<()> {
...
pub async fn remove_user_folder(db_pool: &Pool, user_id: i32, folder_id: i32) -> Result<()> {
    delete(
        user_folders::table.filter(
            user_folders::id.eq(folder_id).and(user_folders::user_id.eq(user_id))
        ),
    )
    .execute_async(db_pool)
    .await?;
